...
def beam_search(
    ...
    while not terminated(beams):
        ...
        for i in range(len(beams)):
            ...
            new_beams.extend([
                Beam(
                    [*beams[i].beam, candidates[j]],            # add candidate to beam
                    beams[i].score * scores[j],                 # multiply score by candidate score
                    candidates[j] == tokenizer.eos_token or     # done if eos_token or hit max length
                    beams[i].length + 1 >= max_gen_length,
                    beams[i].length + 1                         # increment beam length by 1
                )
                for j in range(len(candidates))
                if candidates[j] != beams[i].beam[-1]           # no duplicate tokens
            ])
        
        # sort new beams descending
        if parser != None:
            new_beams.sort(
                key=lambda beam: beam.score + parser.score(" ".join(beam.beam)), 
                reverse=True
...
tokenizer_orig.add_special_tokens(special_tokens) # with this, you don't have to manually define the new tokens' ids
...
cs=[
    {'start': 'Venus', 'end': 'a planet', 'relationship': 'IsA', 'weight': 9.38083151964686},
    {'start': 'earth', 'end': 'planet', 'relationship': 'RelatedTo', 'weight': 7.979724306014589},
    {'start': 'saturn', 'end': 'a planet', 'relationship': 'IsA', 'weight': 6.0},
    {'start': 'Mercury', 'end': 'the planet', 'relationship': 'IsA', 'weight': 6.0},
    {'start': 'Uranus', 'end': 'a planet', 'relationship': 'IsA', 'weight': 5.656854249492381},
    {'start': 'a planet', 'end': 'outer space', 'relationship': 'AtLocation', 'weight': 5.656854249492381},
    {'start': 'Jupiter', 'end': 'a planet', 'relationship': 'IsA', 'weight': 5.656854249492381},
    {'start': 'A planet', 'end': 'a large object', 'relationship': 'IsA', 'weight': 5.291502622129181},
    {'start': 'a planet', 'end': 'orbit', 'relationship': 'AtLocation', 'weight': 4.898979485566356},
    {'start': 'a planet', 'end': 'the universe', 'relationship': 'AtLocation', 'weight': 4.898979485566356},
    {'start': 'a planet', 'end': 'live on', 'relationship': 'UsedFor', 'weight': 4.47213595499958},
    {'start': 'Mars', 'end': 'a planet', 'relationship': 'IsA', 'weight': 4.47213595499958},
    {'start': 'mercury', 'end': 'planet', 'relationship': 'RelatedTo', 'weight': 4.125045454295019},
    {'start': 'Neptune', 'end': 'a planet', 'relationship': 'IsA', 'weight': 4.0},
    {'start': 'a sky', 'end': 'a planet', 'relationship': 'AtLocation', 'weight': 2.82842712474619},
    {'start': 'moon', 'end': 'planet', 'relationship': 'RelatedTo', 'weight': 2.551862065237853},
    {'start': 'planet', 'end': 'astronomy', 'relationship': 'HasContext', 'weight': 2.0},
    {'start': 'superior planet', 'end': 'planet', 'relationship': 'IsA', 'weight': 2.0}
]
import sys
sys.path.append("..")
